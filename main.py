...
def lossFunction(output, target, c_s, use_cuda):
    if c_s == 0:
        ...
        s_target = net.get_feature('ReLU', 3, 1, target).clone()
        s_output = net.get_feature('ReLU', 3, 1, output).clone()
        if use_cuda:
...
def train(model, dataloader, optimizer, epoch, n_epochs, use_cuda, c_s):
    ...
    for data in dataloader:
        ...
        loss = lossFunction(outputs, y_train, c_s, use_cuda)
        loss.backward()
